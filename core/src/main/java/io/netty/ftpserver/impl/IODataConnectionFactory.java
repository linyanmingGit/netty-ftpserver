...
public class IODataConnectionFactory implements ServerDataConnectionFactory {
    ...
    public IODataConnectionFactory(final FtpChannel channel) {
        this.channel = channel;
        if ((channel != null) && (channel.getListener() != null) &&
                channel.getListener().getDataConnectionConfiguration().isImplicitSsl()) {
            secure = true;
        }
    }
    ...
    public synchronized void closeDataConnection() {
        ...
        if (servSoc != null) {
            ...
            if (channel != null) {
                DataConnectionConfiguration dcc = channel.getListener()
    ...
    public synchronized InetSocketAddress initPassiveDataConnection()
        ...
        int passivePort = channel.getListener()
     ...
     * (non-Javadoc)
     * 
     * @see io.netty.ftpserver.FtpDataConnectionFactory2#getInetAddress()
     */
     ...
     * (non-Javadoc)
     * 
     * @see io.netty.ftpserver.FtpDataConnectionFactory2#getPort()
     */
    public int getPort() {
        return port;
    }
     ...
     * (non-Javadoc)
     * 
     * @see io.netty.ftpserver.FtpDataConnectionFactory2#openConnection()
     */
    public DataConnection openConnection() throws Exception {
        return new IODataConnection(createDataSocket(), channel, this);
    }
    ...
    private synchronized Socket createDataSocket() throws Exception {
        ...
        dataSoc = null;
        DataConnectionConfiguration dataConfig = channel.getListener()
                .getDataConnectionConfiguration();
        try {
            if (!passive) {
                if (secure) {
                ...
                if(localAddr == null) {
                    localAddr = channel.localAddress().getAddress();
                }       
                ...
                if (dataConfig.isPassiveIpCheck()) {
					...
					InetAddress remoteAddress = channel.remoteAddress().getAddress();
     ...
     * (non-Javadoc)
     * 
     * @see io.netty.ftpserver.DataConnectionFactory#isSecure()
     */
     ...
     * (non-Javadoc)
     * 
     * @see io.netty.ftpserver.DataConnectionFactory#isZipMode()
     */
    ...
    public synchronized boolean isTimeout(final long currTime) {
        ...
        int maxIdleTime = channel.getListener()
